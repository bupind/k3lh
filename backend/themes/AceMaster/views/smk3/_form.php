...
<div class="row smk3-form">
    <div class="col-xs-12 col-md-6 col-md-offset-3">
        ...
        echo $form->field($model, 'smk3_auditor', ['template' => AppConstants::ACTIVE_FORM_TEMPLATE_INPUT_COL_9_FULL])
                        ...
                        <tr>
                            ...
                            <?php if($model->getIsNewRecord()){ ?>
                                <td colspan="2" class="text-center">
                                    ...
                                    <?= Html::radio($answerName, false, ['value' => '0', 'class' => 'radio-inline'])?>
                                </td>
                                <td colspan="2" class="text-center">
                                    <?= Html::radio($answerName, true, ['value' => '1', 'class' => 'radio-inline'])?>
                                </td>
                                <td colspan="2" class="text-center">
                                    <?= Html::radio($answerName, true, ['value' => '2', 'class' => 'radio-inline'])?>
                                </td>
                            <?php } else{ ?>
                                <?php foreach ($criteria->smk3Details as $key3 => $detail): ?>
                                    <?php if ($model->id == $detail->smk3_id) { ?>
                                        <td colspan="2" class="text-center">
                                            <?= Html::activeHiddenInput($detail, "[$key][$key1][$key2]smk3_criteria_id"); ?>
                                            <?= Html::activeHiddenInput($detail, "[$key][$key1][$key2]id"); ?>
                                            <?= Html::activeRadio($detail, "[$key][$key1][$key2]sdtl_answer", ['label' => '', 'value' => 0, 'uncheck' => null]); ?>
                                        </td>
                                        <td colspan="2" class="text-center">
                                            <?= Html::activeRadio($detail, "[$key][$key1][$key2]sdtl_answer", ['label' => '', 'value' => 1, 'uncheck' => null]); ?>
                                        </td>
                                        <td colspan="2" class="text-center">
                                            <?= Html::activeRadio($detail, "[$key][$key1][$key2]sdtl_answer", ['label' => '', 'value' => 2, 'uncheck' => null]); ?>
                                        </td>
                                    <?php } ?>
                                <?php endforeach; ?>
                            <?php } ?>
